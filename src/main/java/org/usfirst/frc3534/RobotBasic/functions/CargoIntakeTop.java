...
public class CargoIntakeTop extends FunctionBase implements FunctionInterface{
    ...
    public void process(){
        ...
        switch(this.state) {
            case 0:
                if((Robot.oi.getController1().getBButton() && (!Robot.functionProcessor.cargoIntakeFloor.running && !Robot.functionProcessor.hatchPlace.running)) && ((!Robot.functionProcessor.cargoShoot.running && !Robot.functionProcessor.habLevel3ClimbPart1.running) && (!Robot.functionProcessor.habLevel3ClimbPart2.running && !Robot.functionProcessor.xButtonReset.running))) {
                    this.started();
                    this.state = 10;
                }
                break;
